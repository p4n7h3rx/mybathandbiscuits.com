...
export default function Testimonials() {
    ...
    return (
        ...
        >
            ...
            <div className="absolute top-20 left-10 text-[30rem] font-serif font-black text-white/[0.03] leading-none select-none pointer-events-none">
                “
            </div>

            <div className="max-w-7xl mx-auto px-6 md:px-12 relative z-10">
                <div className="text-center mb-24">
                    <span className="font-sans text-[10px] font-black text-white/30 tracking-[0.5em] uppercase mb-8 block">
                        Guest Journals
                    </span>
                    <h2 className="font-serif font-black text-4xl md:text-7xl text-white tracking-tighter leading-[0.9]">
                        Stories of <br />
                        <span className="text-white italic font-normal opacity-70">Care & Trust.</span>
                    </h2>
                </div>

                <div className="relative max-w-5xl mx-auto">
                    <div className="min-h-[300px] md:min-h-[400px] flex flex-col justify-center items-center text-center">
                        <p className="font-serif text-xl sm:text-2xl md:text-5xl text-white/90 leading-[1.2] mb-16 tracking-tight transition-all duration-700 ease-out font-normal">
                            {current.text}
                        </p>
                        ...
                        <div className="flex flex-col items-center gap-4">
                            <div className="flex gap-1.5 mb-2">
                                {Array.from({ length: 5 }).map((_, i) => (
                                    <HiStar key={i} className="text-white/40" size={16} />
                                ))}
                            </div>
                            <h4 className="font-sans text-[10px] font-black tracking-[0.3em] uppercase text-white/30">
                                — {current.author}
                            </h4>
                        </div>
                    </div>
                    ...
                    <div className="flex items-center justify-center gap-12 mt-24">
                        <button
                            onClick={prev}
                            className="group flex items-center gap-4 text-white/20 hover:text-white transition-all cursor-pointer"
                        >
                            <span className="text-[10px] font-sans font-black tracking-widest uppercase">Prev</span>
                            <span className="w-12 h-[1px] bg-white/10 group-hover:bg-white group-hover:w-20 transition-all duration-500" />
                        </button>
                        ...
                        <div className="flex gap-4">
                            {TESTIMONIALS.map((_, index) => (
                                <button
                                    key={index}
                                    onClick={() => goTo(index)}
                                    className={`w-1.5 h-1.5 rounded-full transition-all duration-500 cursor-pointer ${index === activeIndex ? 'bg-white scale-150' : 'bg-white/10'
                                        }`}
                                />
                            ))}
                        </div>
